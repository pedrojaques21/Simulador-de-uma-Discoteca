...
void str_echo (int sockfd) {
    int n = 0;
    char linha[MAXLINE];
    while(!fim_simulacao){								//Enquanto a simulacao estiver a ocorrer
        n = read(sockfd, linha, MAXLINE);			//n guarda o tamanho do line (buffer)
        if(n==0){									// nao ha mensagem a ler
        ...
        else{							//ha mensagem a ler
            if(linha[0]=='1'){
                printMensagem();
            }
            printf("%s\n", AZ);				//imprime a mensagem
        }
    }
}
